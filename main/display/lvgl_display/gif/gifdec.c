...
static gd_GIF* gif_open(gd_GIF* gif_base) {
    ...
    GIFDEC_FILL_BG(gif->canvas, gif->width * gif->height, 1, gif->width * gif->height, bgcolor,
                   0x00);
#else
    for (int i = 0; i < gif->width * gif->height; i++) {
        ...
        gif->canvas[i * 4 + 3] = 0x00;  // Initialize to transparent, let the first frame render
                                        // according to its own transparency settings
    }
#endif
...
static void render_frame_rect(gd_GIF* gif, uint8_t* buffer) {
    ...
    GIFDEC_RENDER_FRAME(&buffer[i * 4], gif->fw, gif->fh, gif->width, &gif->frame[i],
                        gif->palette->colors, gif->gce.transparency ? gif->gce.tindex : 0x100);
#else
    int j, k;
    uint8_t index, *color;
...
void gd_render_frame(gd_GIF* gif, uint8_t* buffer) { render_frame_rect(gif, buffer); }
