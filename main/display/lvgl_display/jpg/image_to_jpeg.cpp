...
static uint8_t* convert_input_to_encoder_buf(const uint8_t* src, uint16_t width, uint16_t height,
                                             ...
                                             int* out_size) {
    // GRAY directly input as JPEG_PIXEL_FORMAT_GRAY
    ...
    // V4L2 YUYV (Y Cb Y Cr) can be directly input as JPEG_PIXEL_FORMAT_YCbYCr
    ...
    // Current version temporarily does not have UYVY format
    ...
    // Current version temporarily does not have YUV422P format
    ...
    // Convert RGB to YUV422 (YCbYCr) and then input
    // See https://github.com/78/xiaozhi-esp32/issues/1380#issuecomment-3497156378
        ...
        switch (format) {
            ...
            case V4L2_PIX_FMT_RGB565:
                ...
                break;
            [[unlikely]] case V4L2_PIX_FMT_RGB565X:  // Current version temporarily does not have
                                                     // RGB565X
...
static uint8_t* convert_input_to_hw_encoder_buf(const uint8_t* src, uint16_t width, uint16_t height,
    ...
    if (format == V4L2_PIX_FMT_YUYV) {
        // Hardware requires "big endian" format of | Y1 V Y0 U |, so bswap16 is needed
        int sz = (int)width * (int)height * 2;
